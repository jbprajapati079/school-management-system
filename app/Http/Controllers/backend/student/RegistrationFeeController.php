...
class RegistrationFeeController extends Controller
{
    public function registrationfeeview(){
        ...
        return view('backend.student.registrationfee.registrationfeeview',compact(['StudentYear','StudentSetup']));
    }
    ...
    public function getstudents(Request $request){
        ...
        foreach ($allStudent as $key => $v) {
            $registrationfee = FeeAmount::where('fee_category_id','3')->where('class_id',$v->student_class_id)->first();
            $color = 'success';
            ...
            $html[$key]['tdsource'] .='<a class="btn btn-sm btn-'.$color.'" title="PaySlip" target="_blanks" href="'.route("student.registration.fee.details").'?class_id='.$v->student_class_id.'&student_id='.$v->student_id.'">Fee Slip</a>';
            $html[$key]['tdsource'] .= '</td>';
   ...
   public function feedetails(Request $request){
        ...
        // dd($data->toArray());
        return view('backend.student.registrationfee.details_pdf',compact('data'));
